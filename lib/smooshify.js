 ...
 *
 * - The array contains no more than four items (or MAX_ARRAY_ITEMS),
...
function smooshifyObject(value, options) {
    ...
    let smooshier = SMOOSHIER.MAX_OBJECT_KEYS && keys.length <= SMOOSHIER.MAX_OBJECT_KEYS;
    ...
    if (smooshier) {
        ...
        if (smooshier) {
            ...
            if ((column + result.length + after) <= SMOOSHIER.MAX_COLUMN) {
                return result;
            }
        }
    }
...
function smooshifyArray(value, options) {
    ...
    let smooshier = SMOOSHIER.MAX_ARRAY_ITEMS && value.length <= SMOOSHIER.MAX_ARRAY_ITEMS;
