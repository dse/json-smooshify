...
function smooshifyObject(value, options) {
    ...
    const smooshierFn = options?.smooshier ?? smooshify.smooshier ?? null;

    let smooshier;
    if (smooshierFn) {
        smooshier = smooshierFn(value);
    }
    if (smooshier == null) {
        smooshier = config.maxObjectKeys && keys.length <= config.maxObjectKeys;
...
function smooshifyArray(value, options) {
    ...
    if (smooshier == null) {
        smooshier = config.maxArrayItems && value.length <= config.maxArrayItems;
